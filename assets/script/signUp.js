    ...
    database.ref('emailVerify').orderByChild("email").equalTo(email).once('value', function (snapshot) {
        ...
        if (unique_email) {
            ...
            Email.send({
                ...
                console.log('a')
                if (message === 'OK') {
                    alert('We have sent a mail to this email regarding Verification code. Please Check');
...
function createAccount() {
    ...
    if (/^\d+$/.test(pin) && pin.length > 5) {
        firebase.database().ref('emailVerify/' + t).on('value', function (snapshot) {
            if (snapshot.val().pin == pin) {
                firebase.auth().createUserWithEmailAndPassword(email, pass).then(t => {
